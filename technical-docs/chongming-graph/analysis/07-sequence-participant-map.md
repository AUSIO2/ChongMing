...
| UC-04-A | 处理客户端、应用编排、控制服务、Run 状态组件、图存储、Host | 调用者/组件/存储 | ApplicationService、ControlService、backend/run.ts、GraphStore、HostWorker |
| UC-04-B | Host、Source 读取组件、图服务、DSH 工作适配器、DSH 进程、业务插件、MongoDB | 组件/运行资源/存储 | HostWorker、source.ts/AssetsService、GraphService、dshRunWork、DeepSeekHarness、dsh-business-plugin、Mongo |
| UC-04-C | 审核客户端、图服务入口、Run 状态组件、图存储 | 调用者/组件/存储 | OperationReview、backend/run.ts、GraphStore |
| UC-04-D | 图服务、Run 状态组件、图存储 | 组件/存储 | GraphService、runUpdateProgress/runCreateOutputs、GraphStore |
